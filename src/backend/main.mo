...
actor {
  ...
  // ===== STATE (all stable for persistence) =====

  stable var nextCustomerId = 1;
  stable var nextOrderId = 1;
  stable var nextAppointmentId = 1;
  stable var nextInventoryId = 1;
  stable var nextInvoiceId = 1;
  stable var nextStaffId = 1;
  stable var seeded = false;
  ...
  stable var ownerPassword = "owner@123";
  ...
  public func seedData() : async () {
    ...
    let m1 : Measurements = {
      chest = 102.0; waist = 86.0; hip = 98.0; shoulder = 46.0;
      sleeveLength = 60.0; shirtLength = 76.0; trouserLength = 104.0; neck = 40.0;
    };
    let m2 : Measurements = {
      chest = 96.0; waist = 80.0; hip = 94.0; shoulder = 44.0;
      sleeveLength = 58.0; shirtLength = 74.0; trouserLength = 100.0; neck = 38.0;
    };
    let m3 : Measurements = {
      chest = 88.0; waist = 72.0; hip = 90.0; shoulder = 42.0;
      sleeveLength = 56.0; shirtLength = 72.0; trouserLength = 98.0; neck = 36.0;
    };
  ...
  public query func filterCustomersByName(name : Text) : async [Customer] {
  ...
  public shared func addCustomer(name : Text, phone : Text, email : Text, address : Text, measurements : ?Measurements) : async Customer {
    let c : Customer = {
      id = nextCustomerId;
      name;
      phone;
  ...
  public shared func updateCustomer(id : Nat, name : Text, phone : Text, email : Text, address : Text, measurements : ?Measurements) : async Bool {
    var found = false;
    customers := customers.map(func(c) {
      if (c.id == id) {
        found := true;
        { id; name; phone; email; address; measurements; createdAt = c.createdAt };
      } else { c };
    });
    found;
  };
  ...
  public shared func addOrder(customerId : Nat, customerName : Text, garmentType : GarmentType, fabricName : Text, fabricColor : Text, quantity : Nat, price : Float, advancePaid : Float, dueDate : Int, notes : Text) : async Order {
    let o : Order = {
      id = nextOrderId;
      customerId;
      customerName;
  ...
  public shared func updateOrderStatus(id : Nat, status : OrderStatus) : async Bool {
    var found = false;
    orders := orders.map(func(o) {
      if (o.id == id) {
        ...
        let deliveryDate = if (status == #Delivered) { ?Time.now() } else { o.deliveryDate };
        {
          ...
          garmentType = o.garmentType; fabricName = o.fabricName; fabricColor = o.fabricColor;
          quantity = o.quantity; price = o.price; advancePaid = o.advancePaid;
          dueDate = o.dueDate; deliveryDate; status; notes = o.notes; createdAt = o.createdAt;
        };
      } else { o };
    });
    found;
  ...
  public shared func addAppointment(customerId : Nat, customerName : Text, dateTime : Int, appointmentType : AppointmentType, notes : Text) : async Appointment {
    let a : Appointment = {
    ...
    appointments := appointments.map(func(a) {
      if (a.id == id) {
        ...
        { id = a.id; customerId = a.customerId; customerName = a.customerName;
          dateTime = a.dateTime; appointmentType = a.appointmentType; status; notes = a.notes };
      } else { a };
    });
    found;
  };

  public shared func deleteAppointment(id : Nat) : async Bool {
  ...
  public query func getInventory() : async [FabricInventory] {
    inventory;
  };

  public shared func addInventoryItem(fabricName : Text, color : Text, quantityMeters : Float, pricePerMeter : Float, supplier : Text, reorderLevel : Float) : async FabricInventory {
  ...
  public shared func updateInventoryItem(id : Nat, fabricName : Text, color : Text, quantityMeters : Float, pricePerMeter : Float, supplier : Text, reorderLevel : Float) : async Bool {
    var found = false;
  ...
  public shared func deleteInventoryItem(id : Nat) : async Bool {
  ...
  public shared func createInvoice(orderId : Nat, customerId : Nat, customerName : Text, subtotal : Float, discount : Float, tax : Float, total : Float, paymentMethod : Text) : async Invoice {
    let inv : Invoice = {
      id = nextInvoiceId;
      orderId;
      customerId;
  ...
  public shared func updateInvoicePayment(id : Nat, paymentStatus : PaymentStatus) : async Bool {
    var found = false;
    invoices := invoices.map(func(inv) {
      if (inv.id == id) {
        found := true;
        {
          id = inv.id; orderId = inv.orderId; customerId = inv.customerId;
          customerName = inv.customerName; subtotal = inv.subtotal; discount = inv.discount;
          tax = inv.tax; total = inv.total; paymentStatus; paymentMethod = inv.paymentMethod;
  ...
  public shared func addStaff(name : Text, role : StaffRole, phone : Text, email : Text, department : Text, joinDate : Int, address : Text) : async Staff {
  ...
  public shared func updateStaff(id : Nat, name : Text, role : StaffRole, phone : Text, email : Text, department : Text, joinDate : Int, address : Text) : async Bool {
    var found = false;
  ...
  public shared func deleteStaff(id : Nat) : async Bool {
  ...
  public shared func setStaffPassword(phone : Text, password : Text) : async Bool {
    var found = false;
    staffCredentials := staffCredentials.map(func(c) {
      if (c.phone == phone) { found := true; { phone; password } } else { c };
    });
    if (not found) {
  ...
  public shared func deleteStaffCredentials(phone : Text) : async () {
    staffCredentials := staffCredentials.filter(func(c) { c.phone != phone });
  };

  // ===== OWNER PASSWORD =====

  public query func getOwnerPassword() : async Text {
    ownerPassword;
  };

  public shared func setOwnerPassword(newPassword : Text) : async Bool {
  ...
  public shared func addCustomerPhoto(customerId : Nat, hash : Text) : async Bool {
    var found = false;
  ...
  public shared func deleteCustomerPhoto(customerId : Nat, hash : Text) : async Bool {
    var found = false;
    customerPhotosList := customerPhotosList.map(func(entry) {
      if (entry.0 == customerId) {
        found := true;
